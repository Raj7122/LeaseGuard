...
class RedisClient {
  ...
  async appendEvent(streamName: string, eventData: any): Promise<string> {
    ...
    try {
      ...
      // For mock client (no REDIS_URL), xadd exists but may differ; handle gracefully
      const canUseXAdd = typeof (redis as any).xadd === 'function';
      if (canUseXAdd) {
        const eventId = await (redis as any).xadd(
          streamName,
          '*',
      ...
      // Fallback: emulate stream via list when xadd is unavailable
      const listKey = `stream:${streamName}`;
      const serialized = JSON.stringify({ id: event.eventId, ...event });
      if (typeof (redis as any).lpush === 'function') {
        await (redis as any).lpush(listKey, serialized);
        console.log(`Appended event (emulated) ${event.eventId} to ${listKey}`);
        return event.eventId;
      }
      console.warn('appendEvent fallback: neither xadd nor lpush available');
      return event.eventId;
    } catch (error) {
  ...
  async storeCommand(command: any): Promise<string> {
    ...
    try {
      ...
      const canUseXAdd = typeof (redis as any).xadd === 'function';
      if (canUseXAdd) {
        const commandId = await (redis as any).xadd(
          'commands',
          '*',
      ...
      // Fallback: emulate with list when streams unavailable
      const listKey = `stream:commands`;
      const serialized = JSON.stringify(commandData);
      if (typeof (redis as any).lpush === 'function') {
        await (redis as any).lpush(listKey, serialized);
        console.log(`Stored command (emulated) ${commandData.commandId}`);
        return commandData.commandId;
      }
      console.warn('storeCommand fallback: neither xadd nor lpush available');
      return commandData.commandId;
    } catch (error) {
