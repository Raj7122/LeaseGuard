...
const nextConfig: NextConfig = {
  ...
  webpack: (config, { dev, isServer }) => {
    ...
    if (!dev) {
      try {
        // Make terser optional to avoid hard dependency in hackathon envs
        ...
        // If terser is not available, fall back to default minimizer
      }
    }
    ...
    if (!dev) {
      try {
        // Only add rule if loader is available
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        ...
        // source-map-loader not installed; skip silently
      }
    }
